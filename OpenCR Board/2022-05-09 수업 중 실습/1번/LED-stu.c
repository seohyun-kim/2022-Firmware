...
void MyApp()
{
	...
	while(1){
		...
		else if(isInputValid(2)){ // 2번 버튼 클릭
			for(int i = 1 ; i <= 4 ; i++){
				TurnOnOneLED(i);
			}
			MyDelay(5);
...
//
//	if(BTN1check && BTN1check){ // 둘다 ON 상태
//		return 3;
//	}else if (BTN1check){ // 1 만 켜진 상태
//		return 1;
//	}else if (BTN2check){ // 2 만 켜진 상태
//		return 2;
//	}else return 0; // 둘다 OFF 상태 인 경우 0을 리턴
//}
...
//		}else if (BTN2check){ // 2 만 켜진 상태
//			return 2;
//		} // 둘다 OFF 상태 인 경우는 while문에 갇혀있음
//	}
//
//	return 0; // 지정해 놓은 wait time 이 끝나면 0을 리턴
//}
