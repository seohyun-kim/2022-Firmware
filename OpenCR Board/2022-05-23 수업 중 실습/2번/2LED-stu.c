...
void MyApp()
{
	...
	SetOneButton(2); //Push sw1 set up

	while(1){
		for(int i = 1 ; i <= 4 ; i++){
			if(CheckIfButtonPushedandBack(2) == 1) continue; // 버튼을 눌렀다 뗀 상태일 때
			TurnOnOneLEDb(i, 'I', 1);
		}
		for(int i = 1 ; i <= 4 ; i++){
			if(CheckIfButtonPushedandBack(2) == 1) continue;
			TurnOnOneLEDb(i, 'O', 1);
		}
...
void TurnOnOneLEDb(UINT32 No, char InOut, UINT32 Duration){
	...
	switch(InOut){
		case 'I': // 내부 LED
			while(waitingTime --){
				 if(CheckIfButtonPushedandBack(2) == 1) return;
				 else TurnOnOneLED(No);
			}
			break;
